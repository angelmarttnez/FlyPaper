...
def _fila_evento_a_json_monitor(fila):
    ...
    return {
        ...
        # Nivel de gravedad calculado en el momento del registro ("CRÍTICO", "ALTO", etc.).
...
def registrar_evento_honeypot(respuesta):
    ...
    # Si procesar_login() detectó un ataque de fuerza bruta para esta IP,
    # sobreescribimos el tipo clasificado automáticamente por clasificar_ataque().
    # Después limpiamos la bandera de sesión para no contaminar peticiones futuras.
    if ruta_visitada == "/login" and session.pop("_fuerza_bruta_detectada", False):
        tipo_ataque_detectado = "Fuerza Bruta"
    # ── Fin sobreescritura fuerza bruta ────────────────────────────────────

    # Convertimos cabeceras a dict plano para serialización JSON en la BD.
    cabeceras_peticion = dict(request.headers)

    # Calculamos el nivel de gravedad según el tipo de ataque detectado.
    # Posibles valores: "CRÍTICO", "ALTO", "MEDIO", "BAJO".
...
def procesar_login():
    ...
    # Obtenemos la IP real (igual que en after_request), considerando proxies.
    ip_peticion = request.headers.get("X-Forwarded-For", request.remote_addr or "")
    if "," in ip_peticion:
        ip_peticion = ip_peticion.split(",")[0].strip()

    # Registramos el intento y comprobamos si supera el umbral (>5 en 1 min).
    # Si hay fuerza bruta, el evento quedará marcado con tipo "Fuerza Bruta"
    # por after_request gracias a que guardamos el resultado en la sesión Flask.
    es_fuerza_bruta = registrar_intento_login(ip_peticion)
    if es_fuerza_bruta:
        # Anotamos en la sesión para que after_request pueda sobreescribir
        # el tipo_ataque detectado automáticamente.
        session["_fuerza_bruta_detectada"] = True
    # ── Fin detección fuerza bruta ─────────────────────────────────────────
...
def monitor_api_estadisticas():
    ...
    # Obtenemos el desglose por nivel de gravedad directamente de la BD.
    # obtener_estadisticas() ya agrupa por la columna `gravedad`.
    ataques_por_gravedad = dict(estadisticas_bd.get("ataques_por_gravedad") or {})
    ...
    cuerpo = {
        ...
        # Distribución de eventos por nivel de gravedad: CRÍTICO, ALTO, MEDIO, BAJO.
